import { JSBI, TokenAmount } from '@photonswap/sdk'
import { isAddress } from 'ethers/lib/utils'
import React, { useEffect, useState } from 'react'
import { Text } from 'rebass'
import styled from 'styled-components'
import Circle from '../../assets/images/blue-loader.svg'
import tokenLogo from '../../assets/images/trustWallet.png'
import { CHAIN_NAME, UNI } from '../../constants'
...
import { Break, CardBGImage, CardBGImageSmaller, CardNoise, CardSection, DataCard } from '../Stake/styled'
      ...
      {!attempting && !claimConfirmed && (
        <ContentWrapper gap="lg">
          <ModalUpper>
            ...
            <CardSection gap="md">
              <RowBetween>
                <TYPE.white fontWeight={500}>Claim {mainToken.symbol}</TYPE.white>
                <CloseIcon onClick={toggleClaimModal} style={{ zIndex: 99 }} color="white" />
              </RowBetween>
              <TYPE.white fontWeight={700} fontSize={36}>
                {unclaimedAmount?.toFixed(0, { groupSeparator: ',' } ?? '-')} {mainToken.symbol}
              </TYPE.white>
            </CardSection>
            <Break />
            <CardSection gap="sm">
              {userClaimData?.flags?.isSOCKS && (
                <RowBetween>
                  ...
                  <TYPE.subHeader color="white">{SOCKS_AMOUNT} {mainToken.symbol}</TYPE.subHeader>
                </RowBetween>
              )}
              ...
              {userClaimData?.flags?.isUser && (
                <RowBetween>
                  ...
                  <TYPE.subHeader color="white">{USER_AMOUNT} {mainToken.symbol}</TYPE.subHeader>
                </RowBetween>
              )}
            </CardSection>
          </ModalUpper>
          ...
          <AutoColumn gap="100px" justify={'center'}>
            ...
            {attempting && claimSubmitted && !claimConfirmed && chainId && claimTxn?.hash && (
              <ExternalLink href={getEtherscanLink(chainId, claimTxn?.hash, 'transaction')} style={{ zIndex: 99 }}>
                View transaction on {CHAIN_NAME[chainId]} Explorer
              </ExternalLink>
            )}
          </AutoColumn>

...
import {
  ...
  CardNoise,
  CardBGImage,
} from "../../components/Farm/styled";
import { ButtonPrimary, ButtonEmpty } from "../../components/Button";
import StakingModal from "../../components/Farm/FarmStakingModal";
import { useFarmingInfo } from "../../state/farm/hooks";
import UnstakingModal from "../../components/Farm/FarmUnstakingModal";
import ClaimRewardModal from "../../components/Farm/FarmClaimRewardModal";
...
export default function FarmManage({
  ...
  const stakingInfos = useFarmingInfo(
    stakingTokenPair,
    tokenA,
  ...
  if (totalSupplyOfStakingToken && stakingTokenPair && stakingInfo && WETH) {
    // take the total amount of LP tokens staked, multiply by ${WETH.symbol} value of all LP tokens, divide by all LP tokens
    ...
    valueOfUserStakedAmountInWETH = new TokenAmount(
      ...
      JSBI.divide(
        JSBI.multiply(
          JSBI.multiply(
            stakingInfo?.stakedAmount?.raw ?? JSBI.BigInt(0),
            stakingTokenPair.reserveOf(WETH).raw
          ),
          JSBI.BigInt(2) // this is b/c the value of LP shares are ~double the value of the WETH they entitle owner to
        ),
  ...
  const decimal =
    stakingInfo?.rewardToken.decimals > 6
      ? 6
      : stakingInfo?.rewardToken.decimals;
  const countUpAmount = stakingInfo?.earnedAmount?.toFixed(decimal) ?? "0";
  ...
  const valueOfUserStakedAmountInUSDC =
    valueOfUserStakedAmountInWETH &&
    USDPrice?.quote(valueOfUserStakedAmountInWETH);
  ...
  const title = `${currencyA?.getDisplaySymbol(
    chainId
  )}-${currencyB?.getDisplaySymbol(chainId)} Liquidity Mining`;
  return (
    <PageWrapper gap="lg" justify="center">
            ...
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.main fontWeight={600}>
                  Step 1. Get PHOTON Liquidity tokens
                </TYPE.main>
              </RowBetween>
              <RowBetween style={{ marginBottom: "1rem" }}>
                <TYPE.main fontSize={14}>
                  {`PHOTON LP tokens are required. Once you've added liquidity to the ${currencyA?.getDisplaySymbol(
                    chainId
                  )}-${currencyB?.getDisplaySymbol(
                    chainId
              ...
              >
                {`Add ${currencyA?.getDisplaySymbol(
                  chainId
                )}-${currencyB?.getDisplaySymbol(chainId)} liquidity`}
              </ButtonPrimary>
            </AutoColumn>
          </CardSection>
          <CardBGImage />
      ...
      <PositionInfo gap="lg" justify="center" dim={showAddLiquidityButton}>
            ...
            <CardSection>
              ...
              <AutoColumn gap="md">
                ...
                <RowBetween style={{ alignItems: "baseline" }}>
                    ...
                    ${stakingInfo?.stakedAmount?.toSignificant(6) ?? "-"}  
                     (${
                       valueOfUserStakedAmountInUSDC
                         ? `$${valueOfUserStakedAmountInUSDC?.toSignificant(8, {
                             groupSeparator: ",",
                           })}`
                  ...
                  <TYPE.main>
                    {"PHOTON-LP " +
                      currencyA?.getDisplaySymbol(chainId) +
                      "-" +
                      currencyB?.getDisplaySymbol(chainId)}
                  </TYPE.main>
                </RowBetween>
              </AutoColumn>
            </CardSection>
        ...
        {!userLiquidityUnstaked ? null : userLiquidityUnstaked.equalTo(
          ...
          <TYPE.main>
            {userLiquidityUnstaked.toSignificant(6)} {"PHOTON LP"} tokens
            available
          </TYPE.main>
        )}
      </PositionInfo>
